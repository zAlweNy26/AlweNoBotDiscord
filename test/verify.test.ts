...
describe("verifyDiscordSignature", () => {
  ...
  it("rejects a tampered body", async () => {
    ...
    await expect(
      verifyDiscordSignature({ ...signed, body: JSON.stringify({ type: 2 }) }),
    ).resolves.toBe(false);
  });
  ...
  it("rejects a missing signature", async () => {
    ...
    await expect(
      verifyDiscordSignature({ ...signed, signatureHex: null, body: "{}" }),
    ).resolves.toBe(false);
  });
  ...
  it("rejects malformed hex", async () => {
    const signed = await createSignedRequest("{}");
    await expect(
      verifyDiscordSignature({ ...signed, publicKeyHex: "zz", body: "{}" }),
    ).resolves.toBe(false);
  });
});
