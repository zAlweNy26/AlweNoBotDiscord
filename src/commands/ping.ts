...
export const pingCommand: Command = {
  ...
  async execute(context) {
    return runDeferred(context, async () => {
      ...
      for (let attempt = 0; attempt < 20 && replyTimestamp === undefined; attempt += 1) {
        try {
          const reply = (await context.rest.get(
            Routes.webhookMessage(context.env.DISCORD_APPLICATION_ID, context.interaction.token, "@original"),
          )) as APIMessage
      ...
      const latency = (replyTimestamp ?? Date.now()) - snowflakeToDate(context.interaction.id).getTime()
      return {
        embeds: [
          {
            color: SUCCESS_COLOR,
