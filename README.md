...
- `/mention enable` lets the bot answer whoever tags it, in the same voice as the summaries (same Workers AI model, one reply per tag). It is off by default and configured per server; `/mention mostra` shows the current state. The reply needs the gateway to receive `MESSAGE_CREATE`, which the Durable Object subscribes to through the (non-privileged) **Guild Messages** intent — nothing to enable in the portal. Intents are frozen when a gateway session identifies, so the first reconnect after this deploy identifies from scratch instead of resuming.
